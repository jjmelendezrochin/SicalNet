...
namespace UserInterface.Forms.Structures
{
	...
	/// </summary>
	public class Cubas : System.Web.UI.Page
	{
		...
		protected Controls.CubasGrid Cubas1;

		private void Page_Load(object sender, System.EventArgs e)
		{
			if (!Page.IsPostBack)
			{
				cmdFProducto.Attributes.Add("onclick", "if(confirm('¿Está seguro que desea insertar esta cuba?')){}else{return false}"); 
...
//		}
		...
		public void cmdFProducto_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				string sDenomicacion =  this.txtDenominacion.Text;

				SICALNet.BusinessLogicLayer.Cuba BRCuba = new SICALNet.BusinessLogicLayer.Cuba();
				SICALNet.BusinessEntities.CubaInfo OInfo  = new SICALNet.BusinessEntities.CubaInfo(1,int.Parse(idLinea),false,"",sDenomicacion);    
				BRCuba.InsertCuba(OInfo);
				// guardamos en la bitacora
				SICALNet.BusinessLogicLayer.Bitacora  BLLBitacora= new SICALNet.BusinessLogicLayer.Bitacora();
				BLLBitacora.Insertcomando("Alta de Cuba: " + OInfo.Denominacion ,this.User.Identity.Name.ToString());

				//clearControl();
				this.Cubas1.BindGrid();

				prcErrorDisplay(null,"La cuba se agregó existosamente");
