...
namespace UserInterface.Forms.Production
{
	...
	/// </summary>
	public class Confirmacion1 : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				case 1:
					this.lblMensajeOperacion.Text = "Reprogramación";
					break;
				case 2:
					this.lblMensajeOperacion.Text = "Cancelación";
					break;
				case 3:
					this.lblMensajeOperacion.Text = "Modificación";
					break;
				case 4:
					this.lblMensajeOperacion.Text = "Cancelación";
					break;
			...
			{			
				...
				IList RsEquals=(IList) BLLMAPP.SelectMotivoAjustesProgramaProduccion(idOperacion);
		...
		{
			if (this.txtOldMessages.Text.Length <10){
				lblmsg.Text = string.Format("Favor de capturar el nombre del operador con al menos 10 caractéres");
				this.lblmsg.Visible=true;
				return;
			}

			if (this.txtNewMessage.Text.Length <15)
			{
				lblmsg.Text = string.Format("Favor de capturar el motivo de cambio con al menos 15 caractéres");
				this.lblmsg.Visible=true;
				return;
			}
			...
			if(lblOperacion.Text=="1")
			{
				...
				this.lblmsg.Visible=true;
				lblmsg.Text = string.Format("La secuencia {0} se insertó con éxito {1} ", lblSecueciaNo.Text, Environment.NewLine);
				...
				BLLMotivos.InsertaMotivo(0, int.Parse(Request.QueryString["Linea"].ToString()),int.Parse(this.cboMotivo.SelectedValue), int.Parse(this.lblOperacion.Text),this.txtNewMessage.Text, this.lblSecueciaNo.Text,this.txtOldMessages.Text, this.User.Identity.Name.ToString());
			...
			if(lblOperacion.Text=="2")
			{
				...
				foreach (string secuencia in split) 
				{
					if (secuencia.Trim() != "")
					{
						...
						this.DelSequence(secuencia, sBitacora);
			...
			if(lblOperacion.Text=="3")
			{
				...
				this.lblmsg.Visible=true;
				lblmsg.Text =  string.Format("La secuencia {0} se actualizó con éxito {1} ", lblSecueciaNo.Text, Environment.NewLine);
				// guardamos en la bitacora
				string sBitacora = string.Format("La secuencia {0} {1},  se actualizó por el usuario {2}, por el motivo {3}",lblSecueciaNo.Text, this.lblValores.Text, this.txtOldMessages.Text, this.txtNewMessage.Text);
				SICALNet.BusinessLogicLayer.Bitacora  BLLBitacora= new SICALNet.BusinessLogicLayer.Bitacora();
				BLLBitacora.Insertcomando(sBitacora,this.User.Identity.Name.ToString());
			...
			if(lblOperacion.Text=="4")
			{
				...
				this.lblmsg.Visible=true;
				lblmsg.Text = string.Format("La secuencia {0} se eliminó con éxito {1} ", lblSecueciaNo.Text, Environment.NewLine);
		...
		private void DelSequence(string Sequence, string sComentario)
		{
			...
			SICALNet.BusinessLogicLayer.MotivosAjusteProgProd BLLMotivos = 
				new SICALNet.BusinessLogicLayer.MotivosAjusteProgProd();				
			BLLMotivos.InsertaMotivo(0, int.Parse(Request.QueryString["Linea"].ToString()),int.Parse(this.cboMotivo.SelectedValue),int.Parse(this.lblOperacion.Text),this.txtNewMessage.Text, Sequence,this.txtOldMessages.Text, this.User.Identity.Name.ToString());
		}

		private void txtNewMessage_TextChanged(object sender, System.EventArgs e)
		{		
		}
